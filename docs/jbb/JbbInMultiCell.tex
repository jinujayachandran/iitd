...
\begin{eqnarray}
	V_5^{BT} &\triangleq& \rho_o\mathbb{E}\left[\Bigm|\sum_{j=1}^{L}\pmb{g}_{lkj}^T\Pi^{\perp}_{{\hat{\pmb{G}}_j}}\pmb{U}_j \pmb{q}_{j}\Bigm|^2\right] 
            ...
            &=&\frac{\rho_o}{M'}\sum_{j=1}^{L}\sum_{i=1}^{L} \mathbb{E}\left[\pmb{g}_{lkj}^{T}\Pi^{\perp}_{\pmb{\hat G}^*_j}\pmb{U}_j^*\pmb{q}_j^*\pmb{q}^T_i\Pi^{\perp}_{\pmb{\hat{G}}^T_i}\pmb{U}_i^T\pmb{g}_{lki}^* \right] \nonumber
\end{eqnarray}
...
\end{eqnarray}
 The initial power control coefficients, $\hat \eta_{lk}$, is obtained under the assumption that the coherent interference term is negligible in each cell. Thus from (\ref{eqn:sinrlkbt})
 \begin{eqnarray}
 \hat \eta_{lk} &=& \frac{\pmb{SINR}_{lk}\left(\rho_b\sum\limits_{j=1}^{L}\beta_{lkj}+M\rho_b\sum\limits_{\substack{j=1 \\ j \neq l}}^{L}\beta_{lkj}\eta_{jk} +\rho_o\left(\sum\limits_{j=1}^{L}\beta_{lkj} -\gamma_{lkl}\right)+1\right)}{M\rho_b\eta_{lk}\gamma_{lkl}}
 \end{eqnarray}
 Using the constraint (\ref{eqn:etaconstraint}), the value of  $\hat \eta_{lk}$ is given by
...
\end{eqnarray}
Using $\hat \eta_{lk}$, the exact value of the signal to interference noise ratio, $\pmb{\widehat{SINR}}_{lk}^{MR}$, is obtained. 
\begin{eqnarray}
