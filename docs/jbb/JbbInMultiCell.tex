...
\end{eqnarray}
Substituting the values for $\pmb{g}_{lkl}$ and $\pmb{y}_{lk}^{p}$ in (\ref{eqn:alphammse}) and taking the required expectation we obtain
\begin{eqnarray}\label{eq:alphapll}
	\alpha_{lk}=\frac{\sqrt{\rho_p \tau_p} \beta_{lkl}}{1+\rho_p\tau_p\sum\limits_{j=1}^{L}\beta_{jkl}}
\end{eqnarray}
and the estimate of the channel from the $k$-th BT of the $l$-th cell to the BS of the same cell is given by
...
\begin{eqnarray}
	V_5^{BT} &\triangleq& \rho_o\mathbb{E}\left[\Bigm|\sum_{j=1}^{L}\pmb{g}_{lkj}^T\Pi^{\perp}_{{\hat{\pmb{G}}_j}}\pmb{U}_j \pmb{q}_{j}\Bigm|^2\right] 
            ...
            &=&\frac{\rho_o}{M'}\sum_{j=1}^{L}\sum_{i=1}^{L} \mathbb{E}\left[\pmb{g}_{lkj}^{T}\Pi^{\perp}_{\pmb{\hat G}_j}\pmb{U}_j\pmb{q}_j^*\pmb{q}^T_i\pmb{U}_i^H\Pi^{\perp}_{\pmb{\hat{G}}_i}\pmb{g}_{lki}^* \right] \nonumber
\end{eqnarray}
...
\end{eqnarray}
 ...
 \begin{eqnarray}
 \hat \eta_{lk} &=& \frac{\pmb{SINR}_{lk}\left(\rho_b\sum\limits_{j=1}^{L}\beta_{lkj}+M\rho_b\sum\limits_{\substack{j=1 \\ j \neq l}}^{L}\beta_{lkj}\eta_{jk} +\rho_o\left(\sum\limits_{j=1}^{L}\beta_{lkj} -\gamma_{lkl}\right)+1\right)}{M\rho_b\gamma_{lkl}}
 \end{eqnarray}
 Using constraint (\ref{eqn:etaconstraint}), the value of  $\hat \eta_{lk}$ is given by
...
where
\begin{eqnarray}
	\pmb{h}_{ejl} &\triangleq& \pmb{U}_j^T\Pi^{\perp}_{{\hat{\pmb{G}}^T_j}}\pmb{h}_{jl} = \pmb{U}_j^T\pmb{h}_{jl} \\
...
\begin{eqnarray}\label{eqn:otyt}
 y_{ol}(t) = \sqrt{\rho_o} \sum_{j=1}^{L} \pmb{h}^T_{ejl} \pmb{q}_j(t)
            + \sqrt{\rho_b'} \sum_{j=1}^{L} \pmb{h}^T_{jl} \left(\sum_{k'=1}^{K} \pmb{v}_{jk'} s_{jk'}(t)\right) + w_{ol}(t)  
\end{eqnarray} 
Substituting $\pmb{h}_{ell} \triangleq \pmb{\hat h}_{ell}-\pmb{\widetilde{h}}_{ell}$, the equation (\ref{eqn:otyt}) can be written as
...
\begin{eqnarray}
     V_{3l}^{OT} &\triangleq& \rho_b' \mathbb{E}\left[\sum_{k'=1}^{K} \pmb{v}_{lk'}^H \pmb{h}_{ll}^*  \pmb{h}_{ll}^T\pmb{v}_{lk'}| \pmb{\hat h}_{ell} \right] \nonumber \\
            &=& \rho_b' \mathbb{E}_{\pmb{\hat G}_l}\left[\mathbb{E}\left[\sum_{k'=1}^{K} \pmb{v}^H_{lk'} \pmb{h}_{ll}^* \pmb{h}_{ll}^T \pmb{v}_{lk'}| \pmb{\hat h}_{ell},  \pmb{\hat G}_{l} \right] | \pmb{\hat h}_{ell}\right] \nonumber \\
            &=& \rho_b' \mathbb{E}_{\pmb{\hat G}_l}\left[\sum_{k'=1}^{K}\pmb{v}^H_{lk'} \mathbb{E}\left[ \pmb{h}_{ll}^*  \pmb{h}^T_{ll} | \pmb{\hat h}_{ell},  \pmb{\hat G}_{l} \right] \pmb{v}_{lk'}| \pmb{\hat h}_{ell}\right]
\end{eqnarray}
